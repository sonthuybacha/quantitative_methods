...
A sample is considered consistent if with increasing sample size the estimate approaches the true value (i.e., the estimate gets more accurate). Assume that we drew many samples with increasing sample size from a known population. We could estimate the mean and compare it to the 'true' mean:

```{r echo=FALSE, results='asis', warning=FALSE, message=FALSE, fig.height=4, fig.width=4, fig.align='center'}

pop <- rnorm(10000, 55, 15)
...
ggplot(data = data.frame(n = seq(10, 10000, 10),
                         DBH = x), 
       aes(x = n, y = DBH)) +
  geom_point() +
...
A sample is un-biased when the deviance between the estimated and the expecte value is zero. Assume that we drew 10,000 samples (each n=150) from a known population and estimate the mean for each sample. Following, we calculated the difference between the known 'true' value and the estimate. The result yields following distribution:
...
for (i in 1:10000) {
  x <- c(x, mean(sample(pop, 150)))
}

ggplot(data = data.frame(x = x - mean(pop)), 
       aes(x = x)) +
  geom_histogram() +
...
dbh_repeat <- replicate(1000000, rnorm(150, 55, 15))
dbh_repeat_mean <- apply(dbh_repeat, 2, mean)
p1 <- ggplot(data.frame(DBH = dbh_repeat_mean), aes(x = DBH)) + geom_histogram() + xlim(35, 75)

dbh_repeat2 <- replicate(1000000, rnorm(15, 55, 15))
dbh_repeat2_mean <- apply(dbh_repeat2, 2, mean)

p2 <- ggplot(data.frame(DBH = dbh_repeat2_mean), aes(x = DBH)) + geom_histogram() + xlim(35, 75)
...
This little experiment demonstrated us that we yield slightly different estimates each time we sample from a population! The variation in sampling means depends on the (population) variance and the sample size. We can calculate the variation in sampling means by:
...
Beware that precision and accuracy are two different things. Your estimate might be very precise, but not accurate. Accuracy hereby is defined as the difference between an estimate and the 'true' value. If there is a systematic difference between the an estimate and the 'true' value, we call this a bias.
...
Calculating the p-value we get $p y 0.000001$, indicating a highly significant difference. The difference in means, however, is only 0.5 $mg L^{-1}$, which is close to the measurement error.
