...
A sample is considered consistent if with increasing sample size the estimate approaches the true value (i.e., the estimate gets more accurate). 
...
A sample is un-biased when the deviance between the estimated and the expecte value is zero. 

Assume that we drew 10,000 samples (each n=150) from a known population and estimate the mean for each sample. Following, we calculated the difference between the known 'true' value and the estimate. The result yields following distribution.
...
Beware that precision and accuracy are two different things. Accuracy hereby is defined as the difference between an estimate and the 'true' value. Your estimate might be very precise, but not accurate. In turn, your estimate might be very accurate, but very unprecise.
